...
`--style fugue` treats the first qualifying idea as an immutable origin and keeps a separate shared theme that can evolve through the exchange. Each completed phrase is compared with the audible part of Jev's preceding answer, your preceding phrase, the shared theme, and the origin. The engine chooses whether to acknowledge, continue, question, support, settle, or wait before selecting a one-voice sequence, inversion, fragmentation, augmentation, or return.
...
Play a concise two-to-six-note idea, then leave enough silence for it to be recognized as a phrase. The first answer preserves its timing and contour on the next safe half-beat opportunity. Try answering a fragment of Jev's line, repeating your own idea, or offering a clearly different idea: those cases update different parts of the conversation state. After an unanswered response, Jev may make one short proposal and then waits. The unchanged origin remains available for a measured return. A short pluck, organ, harpsichord-like patch, or restrained strings keeps the one-voice exchange clearer than a long pad.
...
The performance view uses one shared engraved surface. Human pitch, onset, duration, and rests form a large ivory contour. Jev reveals its actual response notes in the same time and pitch coordinates as their scheduled onset passes; sequence, inversion, fragmentation, augmentation, and return therefore retain their audible geometry instead of receiving a decorative symbol. The response head is the only persistent acid-green accent. Kick briefly compresses the surface, rim click shears it, hi-hat makes a short incision, and keyboard punctuation creates a local interference pattern. These rhythm events never use drum-note numbers as melodic height and never overwrite the remembered phrase.
...
For `--style fugue`, a finalized motif enters the local conversation engine instead of the legacy fixed eight-bar development cycle. The first response is deliberately conservative: one modal voice on the next safe half-beat opportunity, preserving recognizable contour, relative rhythm, rests, and accents while shaping articulation and velocity into one phrase. Register placement is chosen once for the whole phrase, so an ascending subject cannot become a large descending jump merely because one note crossed an octave boundary. The candidate set is bounded to silence, a close echo, a cadential tail variation, and a held tone; the local arbiter may choose silence when no safe recognizable answer exists.

After the first echo, development is selected from the live relationship rather than a stage clock. Transposition and inversion operate in mode degrees before phrase-wide register placement. Dense playing invites augmentation, register displacement invites a modal sequence, close contour repetition invites inversion, and longer material can be fragmented around its opening, salient interior, and ending. Candidates are measured from their actual pitches and timing for contour integrity, interval recognition, modal fit, cadence, continuity from the last heard response, space, and voice leading. Each answer records its origin, current human source, rendered response, and motif lineage. Conversation memory advances only when a companion Note On's scheduled onset has elapsed; a response canceled before sounding does not become musical history. Once measured transformation distance crosses the limit, the engine can restate the immutable origin and begin a new arc.

With `--brain jev`, the conversation engine sends one request only after a motif is complete. The request contains musical context plus each bounded candidate's pitches, intervals, relative onsets, durations, landing quality, continuity, and scores; Jev still cannot invent notes or MIDI events. The live queue polls the prepared result without waiting. A valid answer reserves a future shared beat, while timeout, low confidence, malformed output, an unknown ID, or a superseded revision selects the deterministic local candidate. Traces are written to stderr as `jev-conversation-trace` JSON without the API key.

The browser receives motif lineage only after the matching MIDI message crosses the 50 ms commitment horizon. It draws the human source and the revealed portion of the response as related curve fields: sequence relocates the flow, inversion reverses its melodic direction, fragmentation opens gaps, augmentation spreads time, and original return increases density while realigning with the origin. Per-note polygons are not used. The tiny clock and control affordance remain almost transparent until hover or keyboard focus.

The evaluation command records this conversation path deterministically. Use the same fixture, sound patch, and volume when comparing local and Jev selection so candidate quality and selection quality remain separable.
