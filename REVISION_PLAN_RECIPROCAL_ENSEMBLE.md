...
| 段階 | コードと自動検証 | 実機・体験検証 |
| --- | --- | --- |
| R0 基準・評価 | シナリオ表、trace v3、検証記録を実装。CI #64成功 | 同条件のrules/Jev録音比較は未実施 |
| R1 共通実行・履歴 | rolling schedulerの予定・取消と発音時刻通過を分離して実装・テスト済み | MIDI送信・停止は実機再確認待ち |
| R2 観測・応答関係 | 最大32音の対応付け、移調・輪郭・リズム・音価・応答時間、単音/和音観測、連続演奏分割を実装・テスト済み | 短い返答・和音・レガートは実機再確認待ち |
| R3 共有テーマ・意図 | 原型/共有テーマ/保留テーマ、6意図、ターン状態、一度だけの自発提案を実装・テスト済み | 二往復以上の接続評価待ち |
| R4 譲り合い・時間 | 全Note On取消を廃止し、フレーズ境界の未送信suffix置換、半拍機会、同一開始時刻fallbackを実装・テスト済み | 相づち・重奏・主導権交代は実機評価待ち |
| R5 音楽生成・回帰 | 共有テーマ起点の変形、実音距離、原型回帰完了条件、音域/末尾保全、中心音保留を実装・テスト済み | 音色固定の比較試聴待ち |
| R6 Jev接続 | 会話状態を含むbounded choice、期限・失効・fallback・secret-free traceを実装・テスト済み | 実APIで採用率・遅延・選択品質を記録予定 |
| R7 映像 | session/note/response/source ID、単調時計、遅着追従、取消、再接続、SSE集約、対応線を実装。JS CI成功 | 音付き動画・前景同期の実測待ち |
| R8 統合 | macOS CI #64でbuild、JS構文、99 tests、ASan成功。README/SPEC/検証手順更新済み | 3分の往復評価と30分実機確認が未実施 |
